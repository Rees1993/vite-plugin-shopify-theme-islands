...
Triaged `needs-triage` → `ready-for-agent`. The fold-in of the `lifecycle.ts` rename was approved during grilling, so the slice is fully scoped. Reversible refactor — no ADR. Blocked by retry-scheduler extraction.
