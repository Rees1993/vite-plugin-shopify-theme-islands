...
Triaged `needs-triage` → `ready-for-agent`. Tests-only slice with full acceptance criteria; no architectural decisions left. Helper-shape that 02 reuses lands here.
